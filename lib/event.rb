class Event < ActiveRecord::Base
  ...
  scope :view_last_month, -> { where(:start => Time.now.beginning_of_month..Time.now.end_of_month).sort_by_date }
  scope :view_next_month, -> { where(:start => Time.now.beginning_of_month..Time.now.end_of_month).sort_by_date }
end
